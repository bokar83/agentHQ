...
- **Every session-log entry MUST end with `**Next:**` line.** Morning digest at 07:30 MT scans roadmaps bottom-up for `**Next:** / **Next session:** / **Next studio session:** / **Next session priorities:** / **Next steps:** / **What's next:** / **Next milestone:** / **Up next:**` markers. Without one, the digest line for that roadmap surfaces "No next-actions section found" and Boubacar wakes up to a broken brief. (2026-05-15: 3 of 9 roadmaps (lighthouse, atlas-second-brain-phase-a, homelab) lacked Next markers; digest produced "No next-actions" lines. Fix shipped via `fix/morning-digest-roadmap-coverage`.)
- **Deferred roadmaps must declare it in the header.** `**Status:** deferred` (and optionally `**Earliest reopen:** YYYY-MM-DD`) tells the digest to surface `[DEFERRED] earliest reopen <date>` instead of trying to find a next-action. Pattern set by homelab.md 2026-05-14.
- **Atlas uses A-prefix for new milestones.** A=Atlas, E=Echo, C=Compass, S=Studio, H=Harvest, L=Lighthouse, G=Ghost. (Old M-prefix entries that already shipped or are mid-flight stay as-is; new entries use the prefix convention. Memory: `feedback_milestone_id_prefix_convention.md`.)
