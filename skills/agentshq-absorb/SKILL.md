...
| Same-shape absorb within 7 days of prior PROCEED on same shape | Downgrade verdict to ARCHIVE-AND-NOTE by default unless the new artifact carries a NOVEL meta-pattern not present in the prior PROCEED. Shape = source-type + topical-domain + format (e.g. "X creator + Claude prompts + numbered list"). Example: 2026-05-13 21:15 absorbed @anatoli_kopadze "20 Claude prompts" (PROCEED, 4 enhancements). 2026-05-14 03:17 absorbed @doublenickk "20 Claude Code prompts" (same shape). Most templates overlapped or were already covered; only the compound-workflow stack primitive was novel. Correct outcome on the second: ARCHIVE-AND-NOTE the templates + PROCEED narrowly on the novel meta-pattern only. Falsifiability clause: if this rule does not fire correctly on the next qualifying absorb (3rd same-shape within 7 days), revert the entry. The rule is theory until tested. |
