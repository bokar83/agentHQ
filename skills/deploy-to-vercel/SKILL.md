...
description: "BOUBACAR RULE: Vercel = preview and mobile testing ONLY. NOT for production. Use when user says 'get me a preview link', 'share with client for review', or 'test on mobile'. When a site needs to go LIVE, stop and use hostinger-deploy instead. Never propose Vercel as the production host."
metadata:
  author: vercel
  version: "3.0.0"
---
...
> **BOUBACAR RULE:** Vercel is for **preview deployments and client sharing only**.
> Mobile testing, stakeholder review links — that's Vercel's job here.
> **All production/live deployments go to Hostinger.** Use the `hostinger-deploy` skill for that.
> Never propose Vercel as the live host. If the user says "go live" or "production", redirect to Hostinger.

Deploy any project to Vercel. **Always deploy as preview** (not production) — production is never Vercel.
