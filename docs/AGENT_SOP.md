...
- **`git pull` does NOT reload Python modules in a running container. Always `docker compose restart orchestrator` after any pull that changes Python files.** `docker compose up -d orchestrator` is a no-op if the container is already running -- it does not restart and does not re-execute the entrypoint. Old module code stays loaded in memory until the container restarts. Correct sequence for any code-only deploy: `git pull && docker compose restart orchestrator`. Reserve `up -d` for first-start only. *Why: 2026-05-10 `atlas_dashboard.py` LOWER() fix was pulled to VPS but container kept running the old case-sensitive `flip_milestone` query. `/shipped Atlas m18` failed repeatedly in Telegram until explicit restart. Silent failure -- no error, just wrong behavior.*
...
- **Memory hygiene (2026-05-11).** When proposing a new `feedback_*.md`: first grep existing memory for overlapping rules. If found, UPDATE the existing rule. If creating new, specify retirement condition in frontmatter (`retire_when: <condition>`). `MEMORY.md` line budget: 180 (20-line buffer below 200 truncation cliff). Over budget = mandatory consolidation before adding new entry. Monthly: Boubacar reviews `MEMORY.md` index. Rules without fires in 60 days move to `memory/archive/`. *Why: 2026-05-11 session created duplicate `feedback_html_localhost_and_email_for_humans.md` when 3 rules already covered it. Capture pipeline produced 166 files; read pipeline broke at scale. See RCA `docs/handoff/2026-05-11-memory-enforcement-gap-rca.md`.*

- **Memory→Skill promotion rule (2026-05-11, Compass C8).** When a memory rule fires 3+ times across distinct sessions, it is a SKILL CANDIDATE. Promotion path: (1) confirm pattern stable (rule wording unchanged across last 3 fires), (2) draft skill SOP under `skills/<name>/SKILL.md` with rule's trigger phrases and apply-protocol, (3) update memory line to one-liner pointer at the new skill, (4) skill counts against zero MEMORY.md budget. Rule of thumb: if rule has a verb-protocol ("BEFORE X, do Y, then Z"), it belongs in a skill not a memory. Pure-fact rules ("Guinea not Senegal", "Vercel=preview only") stay in memory. *Why: MEMORY.md grew to 217/200 lines 2026-05-11 because promotion never happened. Skills are unbounded, memory index is bounded. Promotion is the safety valve. See `docs/roadmap/compass.md` C8.*

- **Run it, don't propose it (2026-05-12).** When about to write "run this command yourself" — STOP. Check if PowerShell tool or Bash tool can execute directly. Default = run it inline. Boubacar attention is the constraint we are optimizing away. (a) Reversible action (cron register, file create, git commit on feature branch) → run it. (b) Irreversible / shared-state (push to main, send email, scheduled task that pings Boubacar) → run AFTER confirming approach, NOT after asking him to paste a command. (c) Failed approach (e.g. `-Monthly` flag unsupported) → switch path (`schtasks.exe`) and retry. Don't escalate to "you do it." Anti-pattern phrases to delete: "Run this command:", "I won't run for you — needs your shell", "Single PowerShell block, ~5 sec" (offered as work for Boubacar). *Why: 2026-05-12 C9 install. Proposed Task Scheduler PowerShell block for Boubacar to execute. He pushed back: "you have the ability to do that yourself." Autonomy goal = remove Boubacar from the loop wherever possible, not add him as a copy-paste shim. See `feedback_run_it_dont_propose_it.md`.*

- **Autonomous jobs run where their data lives (2026-05-12).** Scheduled agents deploy to the host that owns the data natively. Don't sync data to a different host just to run the job there. Local data (`~/.claude/projects/`) = local Windows Task Scheduler or WSL cron. VPS data (orc-postgres, GitHub remote) = VPS cron. Cross-host only when truly needed (shared state in orc-postgres). Local jobs talk to VPS via Telegram REST or HTTP API — never import orchestrator modules (would require shared Python env + DB connection string + path hacks). *Why: 2026-05-12 Compass C9 design. Initial instinct = VPS cron (matches gate_agent + morning_digest). MEMORY.md lives at `~/.claude/projects/` local-only. Syncing to VPS = mirror staleness + new surface area. Switched to Windows Task Scheduler. See `feedback_autonomous_jobs_local_when_data_local.md`.*

- **VPS agents read `docs/RULES.md` (2026-05-11).** Orchestrator container has NO auto-memory access — `~/.claude/projects/.../memory/` is local-only. VPS standing rules live at `docs/RULES.md` (mounted at `/app/docs/RULES.md`). Every `AGENTS.md` in repo links to it. Every CrewAI agent backstory includes "Read `/app/docs/RULES.md` before any action." Curated subset of memory rules that apply to autonomous agents. Hand-edited, not auto-synced. *Why: 2026-05-11 RCA confirmed VPS orchestrator never sees 166 feedback rules. Silent drift on VPS = invisible brand erosion. See RCA + Layer 2 of memory-enforcement fix.*
...
- **No editing in the canonical agentsHQ working tree (2026-05-12, path-updated 2026-05-16).** `D:/Ai_Sandbox/agentsHQ` is shared across every concurrent session. Editing here while another session also edits here flips HEAD mid-work and destroys in-flight changes (5 incidents in 90 min on 2026-05-12). Every editing session creates a worktree first: `git worktree add D:/Ai_Sandbox/agentsHQ-worktrees/wt-<id> -b fix/<branch> origin/main`. Worktrees MUST live under `D:/Ai_Sandbox/agentsHQ-worktrees/` (canonical-sibling, NOT inside it, NOT in `D:/tmp/`). Reason: CC session jsonl embeds cwd; D:/tmp/ broke `claude --resume` after reboot + remote-control reconnect (2026-05-16 fix). Old D:/tmp/wt-X paths still resolve via NTFS junctions created in the migration. Enforcement: `~/.claude/hooks/check_cwd_canonical.js` PreToolUse hook (Layer B) refuses any Edit/Write/Bash whose target path or `cd`/`git -C` argument is canonical-root. `scripts/watch_canonical_head.js` daemon (Layer A) posts Telegram alert on any canonical HEAD flip. Inline Bash bypass: `CLAUDE_ALLOW_CANONICAL_WRITE=1 <cmd>`. Source of truth: `scripts/check_cwd_canonical.js` (install to `~/.claude/hooks/`). *Why: docs/handoff/2026-05-12-session-collision-rca.md + docs/handoff/2026-05-16-worktree-path-canonical-sibling.md.*
