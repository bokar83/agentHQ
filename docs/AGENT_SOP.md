...
- **Cold email greeting: drop entirely when first_name is unknown. Never use "Hi there" or any generic salutation.** When `_extract_first_name()` returns confidence "low", templates render NO greeting line; the body opens with the hook (which is already personalized by business_name + city + niche). Cold-email research shows "Hi [first_name]" replies at 3-5%, "Hi there" drops 20% (signals automation), and **no greeting matches "Hi [first_name]"** because the email gets to the point faster and Gmail spam filters do not see the bulk-mail tell. Pattern is implemented in `skills/outreach/sequence_engine.py:_extract_first_name` returning `(name, confidence)` and templates checking `lead["first_name_confidence"]`. Locked 2026-04-30. See `feedback_no_greeting_when_unknown.md`. *Why: Boubacar's standing rule, backed by deliverability data.*
