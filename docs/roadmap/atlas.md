...
**Action set (CEO-Level):**
- **Decision Inbox:** Resolve high-stakes issues, authorize sub-agent spawning, and approve client-facing content/proposals.
- **System Knobs:** Toggle agent autonomy modes (Griot, dry-run, pacing speeds, priority channels).
- **Audit View:** View live coordination queues, heartbeats, logs, and aggregated spend analytics.
...
**Why:** Boubacar's operational goal is to operate strictly as the CEO: agents wake up, coordinate, spawn sub-agents, and execute tasks autonomously, routing only high-level approvals, strategic decisions, or exceptions to this one-tap dashboard.
...
**Why:** Spawning allows the system to remain highly specialized and dynamic. Rather than spinning up full, expensive CrewAI swarms with generic backstories for every micro-task, the orchestrator delegates specific sub-problems to cheap, highly scoped sub-agents that run asynchronously and return results directly to their parent.

**Trigger gate:**
1. Postgres on VPS live + dynamic task queue ready (standard baseline for claims/coordination/minions).
2. Live Agent Graph (M8b) is live (essential to monitor spawned hierarchies).
3. The volume of concurrent sub-tasks justifies a durable background queue (>10 micro-jobs running daily).
...
**Why:** Aligns execution directly with data flow. Fixed cron intervals create latency and unnecessary overhead; an event-driven loop makes the platform truly ambient and "alive," reacting to real-world triggers, client responses, and internal state changes instantly.

**Trigger gate:**
1. Agent-to-Agent Spawning & Delegation (M23) is fully operational.
2. Hermes Self-Healing Execution (M24) is live.
3. The number of background jobs exceeds the capacity of cron-based ticks.

**Branch:** `feat/atlas-m25-event-bus`
