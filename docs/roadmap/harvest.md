...
| # | Task | Why |
|---|---|---|
| T3.1 ⏳ | First "Diagnostic Patterns Q2 2026" post mining Supabase `diagnostic_submissions` for the 5-7 most-common constraints visitors are typing in. | Sankofa Expansionist: aggregate the diagnoses. **2026-05-11 UNBLOCKED — waiting on N:** Supabase access works via `.env` `SUPABASE_URL` + `SUPABASE_SERVICE_KEY` (host `jscucboftaoaphticqci.supabase.co`, REST + service-role). `diagnostic_submissions` schema verified (`pain_text`, `response_json`, `model_used`, `geo_country`, `flagged`, etc.) but **only 10 rows so far**. Need ≥50 rows for a credible insights post — re-attempt once N crosses 50. |
...
| T3.4 ⏳ | Email follow-up sequence wired (3 emails over 14 days after capture) | **2026-05-11 UNBLOCKED — waiting on first capture:** Supabase `diagnostic_captures` table live but **0 rows** so far (no visitor has submitted the email-capture form yet). Sequence wiring can ship preemptively in n8n but won't fire. Defer build until first capture lands so the sequence design fits the actual capture shape. |
| T3.5 ❌ | Stripe direct-payment on Signal Session offer card | **2026-05-11 NOT NEEDED:** Calendly paid event already direct-charges via Stripe. Boubacar's call: keep Calendly funnel as-is. Stripe keys (`sk_live`, `pk_live`, `STRIPE_WEBHOOK_SECRET`) are present in `.env` for future use but no Signal Session product/price exists in Stripe yet (only Signal Works setup-fee tiers). If/when a parallel Stripe-only checkout is wanted, create the product + Payment Link + add button — Stripe access already wired. |
