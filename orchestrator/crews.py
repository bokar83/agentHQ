...
def build_notion_capture_crew(user_request: str) -> Crew:
    ...
    if is_review:
        ...
        task_description = (
            ...
            f"   - Save to the Notion CONTENT BOARD (FORGE_CONTENT_DB) with Status='Idea', "
            f"Content Type='Story', Platform=['LinkedIn','X'], "
            f"Source Note='Signal input from Boubacar — raw story material. Do not polish without his input.'\n"
            f"   - Skip impact/effort scoring. Stories are not scored.\n"
            f"   - Confirm: 'Saved as Story Signal to Content Board. LéGroit will propose a draft when you are ready.'\n"
