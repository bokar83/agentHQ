...
def build_notion_capture_crew(user_request: str) -> Crew:
    ...
    if is_review:
        ...
        task_description = (
            ...
            f"REQUEST: {user_request}\n\n"
            f"SIGNAL DETECTION (run this first):\n"
            f"Before categorizing, determine if this is a STORY SIGNAL — a raw personal thought, "
            f"lived moment, emotional observation, or experience Boubacar is sharing from his own life. "
            f"Signal indicators: first-person past tense ('I did', 'I felt', 'there was a moment'), "
            f"emotional language, specific personal scenes, things that happened to Boubacar directly, "
            f"frustrations, doubts, wins, or observations from his day. "
            f"If it is a story signal: set is_story=True. "
            f"If it is a product/feature/tool idea: set is_story=False.\n\n"
            f"CHANNEL MAPPING (story signals only):\n"
            f"If is_story=True, infer which of Boubacar's 4 channels the story feeds. "
            f"A single story can feed multiple channels — each gets its own Content Board record.\n"
            f"Channel rules:\n"
            f"  BOUBACAR PERSONAL (LinkedIn + X @boubacarbarry): professional journey, consulting, "
            f"Utah business reality, practitioner honesty, firm-building struggle.\n"
            f"  FIRST GEN MONEY (X @FirstGenMoney_ + IG + TikTok + YouTube): money, wealth-building, "
            f"immigrant/first-gen financial reality, what nobody tells you about money and success, "
            f"class mobility, financial decisions and their cost.\n"
            f"  UNDER THE BAOBAB (X @UnderTheBaobab_ + IG + TikTok + YouTube): diaspora identity, "
            f"West African culture, belonging, what it costs to build something far from home, "
            f"family, cultural collision, roots and ambition.\n"
            f"  AI CATALYST (X @boubacarbarry + IG + TikTok + YouTube): AI tools in practice, "
            f"how Boubacar uses AI to run his firm, practical AI for small business operators.\n\n"
            f"Steps:\n"
            ...
            f"3. If is_story=True:\n"
            f"   - Create ONE Content Board record with:\n"
            f"     Status='Idea', Content Type='Story', Platform=['LinkedIn','X']\n"
            f"     Title as extracted (no channel prefix).\n"
            ...
            f"   - After saving, send a Telegram confirmation via notifier.send_message() to "
            f"OWNER_TELEGRAM_CHAT_ID (or TELEGRAM_CHAT_ID) with this format:\n"
            f"     'Story signal saved: \"[title]\"\n\nCould feed: [channels]\n\nReply \"draft it\" when ready.'\n"
            f"   - Confirm to user: record saved, channels suggested, Telegram confirmation sent.\n"
            f"4. If is_story=False:\n"
            f"   - Pick the most fitting category: Tool, Agent, Feature, Business, or Personal.\n"
            f"   - Score on two dimensions (REQUIRED):\n"
            f"     impact (High|Medium|Low): direct revenue/pipeline = High; quality-of-life = Medium; speculative = Low.\n"
            f"     effort (High|Medium|Low): under 2h = Low; half-day = Medium; multi-session = High.\n"
            f"   - Call create_notion_idea with title, content, category, impact, and effort.\n"
            f"   - Confirm: title, scores, Notion URL.\n"
        )
        expected_output = (
            "For story signals: confirmation saved to Content Board as Story with Status=Idea. "
            "For regular ideas: title, category, Impact score, Effort score, and Notion URL. "
