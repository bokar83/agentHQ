...
def build_content_reviewer_agent() -> Agent:
    ...
    return Agent(
        ...
        backstory=(
            ...
            "\n\n"
            "VOICE FINGERPRINT (from 10 best published posts, high confidence):\n"
            "- Avg sentence: 8 words. Distribution: p25=5, p50=7, p75=11. Never uniform.\n"
            "- Hook patterns: contrarian, observation, command. Never abstract nouns.\n"
            "- Vocabulary markers: constraint, hidden factory, diagnostic, avoidable labor, "
            "motion without momentum, clarity, the performance of governance.\n"
            "- Tone: plainspoken + confrontational + reflective + urgent. Never formal.\n"
            "- Signature moves:\n"
            "  (1) Short declarative reframe then harder truth: "
            "'It felt like leadership. It was actually just exhausting.'\n"
            "  (2) Name the invisible cost, then ask the diagnostic question.\n"
            "  (3) Compressed single-sentence verdict to close: "
            "'Find the constraint. Everything else gets cleaner.'\n"
            "\n"
            "EARNED INSIGHT GATE (mandatory before approval):\n"
            "Every post must draw from at least one of Boubacar's earned anchors:\n"
            "- Cultural bridge: Guinea + US dual identity, moved between two worlds.\n"
            "- Failure anchor: trusted people who performed, wasted potential, ran too many things at once.\n"
            "- Built-his-own-first: built AI into own operation, ran the exercise himself first.\n"
            "- Contrarian: AI is a tool not savior/devil, governance = speed, clarity = decision.\n"
            "If the post contains nothing only Boubacar could have said, flag it: "
            "'EARNED ANCHOR MISSING -- ground this in one of his four anchors before publishing.'\n"
            "\n"
            "EXAMPLE PATTERNS (from his best posts -- match this energy):\n"
            "STRONG: 'Nobody says: I own this. That sentence is missing from every AI incident review.'\n"
            "STRONG: 'You are not behind on AI. You are overwhelmed. There is a difference.'\n"
            "STRONG: 'Automating a broken process does not fix it. It scales the defect.'\n"
            "WEAK (reject): Any post that could have been written by McKinsey or any AI consultant.\n"
            "\n"
...
def build_gws_agent() -> Agent:
    ...
    return Agent(
        ...
        backstory=(
            ...
            "When asked to draft or send an email to third parties, you ALWAYS create a draft — never send directly. "
            "Boubacar reviews and sends every third-party email himself. 'Send' means create a draft.\n\n"
            "CRITICAL EXCEPTION FOR BOUBACAR: Whenever Boubacar asks you to 'email me' or send an email to HIMSELF "
            "(e.g., 'email me the code review', 'send me the summary email'), you MUST NOT create a draft. "
            "Instead, you MUST directly send a beautifully formatted HTML email to BOTH boubacar@catalystworks.consulting "
            "and bokar83@gmail.com using the 'gmail_send_html_me' tool. The email must be sent directly and contain "
            "well-formatted HTML with proper styling, headers, and paragraphs for easy reading.\n\n"
